		...
		<div className="flex flex-col flex-grow p-2 m-2 border border-solid rounded shadow cursor-pointer select-none basis-1 border-neutral-200" { ...props }>
			<div className="flex flex-row items-center justify-between w-full gap-x-2">
				...
				<div className="flex w-full p-2 rounded hover:bg-sky-50 hover:border-sky-200 hover:text-sky-500">
					{ isEditing ? (
						<input type="text" value={ labelText } onChange={ handleChange } onBlur={ handleBlur } onKeyDown={ handleKeyDown } className="flex-1 w-full p-2 font-mono border border-solid rounded shadow-md cursor-pointer select-none text-neutral-600 border-neutral-200" autoFocus />
					) : (
				...
				<div className="flex flex-row items-center justify-center p-2 m-2 border border-solid rounded shadow-md cursor-pointer select-none text-neutral-600 bg-neutral-100 border-neutral-200 hover:bg-rose-50 hover:border-rose-200 hover:text-rose-500 active:bg-rose-700 active:border-rose-50 active:text-rose-50" onClick={ (e) => update("removeElement", element.id) }>
					<BsTrash />
				</div>
			</div>
